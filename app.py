...
@st.cache_resource
def load_model():
    # Menggunakan try-except agar jika file hilang, web tidak langsung crash total
    try:
...
try:
    # Pastikan nama file di GitHub adalah water_quality_banner.PNG (Case Sensitive)
    image = Image.open('water_quality_banner.PNG') 
    st.image(image, caption='Sumber Air Bersih', use_container_width=True) 
except Exception as e:
    st.info("Info: Banner gambar belum terdeteksi. Pastikan file 'water_quality_banner.PNG' sudah diunggah.")

if model is None:
    st.error("Gagal memuat model: File 'model_air_terbaik.pkl' tidak ditemukan di GitHub. Silakan upload file tersebut.")
else:
